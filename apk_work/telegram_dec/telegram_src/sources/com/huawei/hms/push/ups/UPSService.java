...
public final class UPSService {
    ...
    public static void turnOffPush(Context context, final UPSTurnCallBack uPSTurnCallBack) {
        ...
        if (s.b()) {
            HmsMessaging.getInstance(context).turnOffPush().addOnCompleteListener(new OnCompleteListener() { // from class: com.huawei.hms.push.ups.UPSService.2
    ...
    public static void turnOnPush(Context context, final UPSTurnCallBack uPSTurnCallBack) {
        ...
        Preconditions.checkNotNull(uPSTurnCallBack);
        if (s.b()) {
            HmsMessaging.getInstance(context).turnOnPush().addOnCompleteListener(new OnCompleteListener() { // from class: com.huawei.hms.push.ups.UPSService.1
                @Override // com.huawei.hmf.tasks.OnCompleteListener
                public void onComplete(Task task) {
                    if (task.isSuccessful()) {
                        uPSTurnCallBack.onResult(new CodeResult());
