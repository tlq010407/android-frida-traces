...
final class ViewingConditions {
    ...
    static ViewingConditions make(float[] fArr, float f, float f2, float f3, boolean z) {
        ...
        double d2 = f;
        Double.isNaN(d2);
        float fCbrt = (f14 * f) + (0.1f * f15 * f15 * ((float) Math.cbrt(d2 * 5.0d)));
        ...
        double d4 = fArr6[0] * fCbrt * f9;
        Double.isNaN(d4);
        float fPow2 = (float) Math.pow(d4 / 100.0d, 0.42d);
        double d5 = fArr6[1] * fCbrt * f10;
        Double.isNaN(d5);
        float fPow3 = (float) Math.pow(d5 / 100.0d, 0.42d);
        double d6 = fArr6[2] * fCbrt * f11;
        Double.isNaN(d6);
        float[] fArr7 = {fPow2, fPow3, (float) Math.pow(d6 / 100.0d, 0.42d)};
        float f16 = fArr7[0];
