...
public class VideoTimeView extends View {
    ...
    public void show(boolean z, boolean z2) {
        ...
        if (z2) {
            ViewPropertyAnimator viewPropertyAnimatorTranslationY = animate().translationY(z ? BitmapDescriptorFactory.HUE_RED : AndroidUtilities.dp(6.0f));
            if (z) {
                f = 1.0f;
            }
            viewPropertyAnimatorTranslationY.alpha(f).scaleX(z ? 1.0f : 0.8f).scaleY(z ? 1.0f : 0.8f).setInterpolator(CubicBezierInterpolator.EASE_OUT_QUINT).setDuration(220L).start();
            return;
        }
