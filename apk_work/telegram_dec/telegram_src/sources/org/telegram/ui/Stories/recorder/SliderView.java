...
public class SliderView extends View {
    ...
    protected void dispatchDraw(Canvas canvas) {
        ...
        rectF.set(BitmapDescriptorFactory.HUE_RED, BitmapDescriptorFactory.HUE_RED, this.w, this.h);
        ...
        if (this.currentType == 0) {
            ...
            animatedTextDrawable = this.text;
            iDp = this.w - AndroidUtilities.dp(111.0f);
            i = -AndroidUtilities.dp(1.0f);
            iDp2 = this.w - AndroidUtilities.dp(11.0f);
        }
        animatedTextDrawable.setBounds(iDp, i, iDp2, this.h - AndroidUtilities.dp(1.0f));
        ...
        if (this.currentType == 0) {
            ...
            float f5 = f3 - f4;
            double d = f5 != BitmapDescriptorFactory.HUE_RED ? f4 + (this.value * f5) : BitmapDescriptorFactory.HUE_RED;
            float f6 = this.wave1Alpha.set(d > 0.25d);
            canvas.save();
            canvas.translate((-AndroidUtilities.dpf2(0.33f)) * (1.0f - f6), BitmapDescriptorFactory.HUE_RED);
            ...
            canvas.save();
            canvas.translate((-AndroidUtilities.dpf2(0.66f)) * (1.0f - f7), BitmapDescriptorFactory.HUE_RED);
            this.speakerWave2Paint.setAlpha((int) (f7 * 255.0f));
            canvas.drawPath(this.speakerWave2Path, this.speakerWave2Paint);
            canvas.restore();
        }
        canvas.save();
        canvas.drawRect(BitmapDescriptorFactory.HUE_RED, BitmapDescriptorFactory.HUE_RED, this.w * f2, this.h, this.whitePaint);
        canvas.restore();
        canvas.restore();
        canvas.restore();
    }
    ...
    public boolean dispatchTouchEvent(MotionEvent motionEvent) {
        ...
        if (motionEvent.getAction() == 0) {
            ...
            float f5 = f3 != BitmapDescriptorFactory.HUE_RED ? f2 + (this.value * f3) : BitmapDescriptorFactory.HUE_RED;
            if (motionEvent.getAction() != 1 || System.currentTimeMillis() - this.pressTime >= ViewConfiguration.getTapTimeout()) {
                this.value = Utilities.clamp(this.value + ((x - this.lastTouchX) / this.w), 1.0f, BitmapDescriptorFactory.HUE_RED);
                this.valueIsAnimated = false;
                z = true;
            } else {
            ...
            float f7 = this.minVolume;
            float f8 = f6 - f7;
            if (f8 != BitmapDescriptorFactory.HUE_RED) {
                f4 = (this.value * f8) + f7;
            }
            if (z) {
                if ((f4 <= f7 && f5 > f4) || (f4 >= f6 && f5 < f4)) {
                    try {
                        performHapticFeedback(3, 1);
                    } catch (Exception unused) {
                    }
                } else if (Math.floor(f5 * 5.0f) != Math.floor(5.0f * f4)) {
