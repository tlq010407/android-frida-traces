...
public class PreviewButtons extends FrameLayout {
    ...
    class ShareButtonView extends View {
        ...
        public ShareButtonView(Context context, String str, boolean z) {
            ...
            this.left = lineCount > 0 ? staticLayout.getLineLeft(0) : BitmapDescriptorFactory.HUE_RED;
            lineWidth = staticLayout.getLineCount() > 0 ? staticLayout.getLineWidth(0) : lineWidth;
        ...
        protected void onDraw(Canvas canvas) {
            ...
            if (zIsPressed) {
                ...
                if (f2 != 1.0f) {
                    float fMin = f2 + (Math.min(40.0f, 1000.0f / AndroidUtilities.screenRefreshRate) / 80.0f);
                    this.pressedProgress = fMin;
                    this.pressedProgress = Utilities.clamp(fMin, 1.0f, BitmapDescriptorFactory.HUE_RED);
                    invalidate();
                }
            }
            float f3 = this.enabledT.set(this.enabled ? 1.0f : 0.5f);
            int saveCount = canvas.getSaveCount();
            if (f3 < 1.0f) {
                canvas.saveLayerAlpha(BitmapDescriptorFactory.HUE_RED, BitmapDescriptorFactory.HUE_RED, getWidth(), getHeight(), (int) (f3 * 255.0f), 31);
            }
            float f4 = ((1.0f - this.pressedProgress) * 0.1f) + 0.9f;
            canvas.save();
            ...
            canvas.save();
            float f5 = (this.w - this.width) / 2.0f;
            if (this.arrow) {
                f = 3.0f;
            }
            canvas.translate((f5 + AndroidUtilities.dp(f)) - this.left, (getHeight() - this.staticLayout.getHeight()) / 2.0f);
    ...
    public PreviewButtons(Context context) {
        ...
        addView(this.shadowView, LayoutHelper.createFrame(-1, -1, R.styleable.AppCompatTheme_windowActionModeOverlay));
    ...
    public void appear(boolean z, boolean z2) {
        ...
        valueAnimatorOfFloat.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() { // from class: org.telegram.ui.Stories.recorder.PreviewButtons$$ExternalSyntheticLambda0
            ...
            public final void onAnimationUpdate(ValueAnimator valueAnimator3) {
                this.f$0.lambda$appear$0(valueAnimator3);
            }
        });
        if (this.appearing) {
            ...
            valueAnimator = this.appearAnimator;
            linearInterpolator = CubicBezierInterpolator.EASE_OUT_QUINT;
        }
