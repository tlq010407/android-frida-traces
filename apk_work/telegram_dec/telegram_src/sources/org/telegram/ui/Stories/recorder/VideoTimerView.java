...
public class VideoTimerView extends View implements FlashViews.Invertable {
    ...
    public void onDraw(Canvas canvas) {
        super.onDraw(canvas);
        float f = this.recordingT.set(this.recording ? 1.0f : BitmapDescriptorFactory.HUE_RED);
        ...
        if (f > BitmapDescriptorFactory.HUE_RED) {
            ...
            double d = jCurrentTimeMillis / 1000.0f;
            Double.isNaN(d);
            paint.setAlpha((int) (Utilities.clamp((((float) Math.sin(d * 3.141592653589793d)) / 4.0f) + 0.75f, 1.0f, BitmapDescriptorFactory.HUE_RED) * 255.0f));
            invalidate();
