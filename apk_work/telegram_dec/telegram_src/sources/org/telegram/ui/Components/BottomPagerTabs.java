...
public abstract class BottomPagerTabs extends View {
    ...
    public class Tab {
        ...
        public Tab(int i, int i2, int i3, int i4, CharSequence charSequence) {
            ...
            this.layoutWidth = lineCount > 0 ? staticLayout.getLineWidth(0) : BitmapDescriptorFactory.HUE_RED;
    ...
    private void setProgress(float f, boolean z) {
        float fClamp = Utilities.clamp(f, this.tabs.length, BitmapDescriptorFactory.HUE_RED);
    ...
    protected void dispatchDraw(Canvas canvas) {
        ...
        if (f > BitmapDescriptorFactory.HUE_RED) {
            double dFloor = Math.floor(this.progress) + 0.5d;
            double d = this.progress;
            Double.isNaN(d);
            double dAbs = (Math.abs(dFloor - d) * 1.2000000476837158d) + 0.4000000059604645d;
            Paint paint = this.selectPaint;
            int color = Theme.getColor(Theme.key_windowBackgroundWhiteBlackText, this.resourcesProvider);
            double d2 = f;
            Double.isNaN(d2);
            paint.setColor(ColorUtils.setAlphaComponent(color, (int) (dAbs * 18.0d * d2)));
            float f5 = width;
            float f6 = f5 / 2.0f;
        ...
        while (true) {
            ...
            tab.setColor(ColorUtils.blendARGB(color2, Theme.getColor(i2, this.resourcesProvider), fMin));
