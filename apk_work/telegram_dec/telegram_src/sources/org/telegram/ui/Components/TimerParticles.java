...
public class TimerParticles {
    ...
    public void draw(Canvas canvas, Paint paint, RectF rectF, float f, float f2) {
        ...
        float f3 = (float) (((-d4) * dWidth) + dCenterX);
        Double.isNaN(dWidth);
        ...
        while (i3 < iClamp) {
            ...
            double d5 = dNextInt * d2;
            if (d5 < 0.0d) {
                d5 += 6.283185307179586d;
            }
            particle.vx = (float) ((Math.cos(d5) * dSin) - (Math.sin(d5) * d4));
            particle.vy = (float) ((Math.sin(d5) * dSin) + (Math.cos(d5) * d4));
            particle.alpha = 1.0f;
            particle.currentTime = BitmapDescriptorFactory.HUE_RED;
            float f6 = 20.0f;
            if (this.big) {
                particle.lifeTime = Utilities.random.nextInt(200) + 600;
                fNextFloat = Utilities.random.nextFloat() * 20.0f;
                f6 = 30.0f;
            } else {
                particle.lifeTime = Utilities.random.nextInt(100) + 400;
                fNextFloat = Utilities.random.nextFloat() * 4.0f;
            }
            particle.velocity = fNextFloat + f6;
