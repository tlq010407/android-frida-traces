        ...
        this.scheduleSwitchToNextRunnable = new Runnable() { // from class: org.telegram.ui.Components.AvatarConstructorPreviewCell.1
            ...
            public void run() {
                ...
                if (avatarConstructorPreviewCell.isAllEmojiDrawablesLoaded || (AvatarConstructorPreviewCell.this.nextAnimatedEmojiDrawable.getImageReceiver() != null && AvatarConstructorPreviewCell.this.nextAnimatedEmojiDrawable.getImageReceiver().hasImageLoaded())) {
                    ...
                    avatarConstructorPreviewCell7.progressToNext = BitmapDescriptorFactory.HUE_RED;
            ...
            if (stickerSets.isEmpty()) {
                ...
                for (int i2 = 0; i2 < featuredEmojiSets.size(); i2++) {
                    ...
                    if (document != null) {
                        ...
                        if (!tL_stickerSetFullCovered.documents.isEmpty()) {
                            this.emojiList.document_id.add(Long.valueOf(((TLRPC.Document) tL_stickerSetFullCovered.documents.get(0)).id));
                        }
                    }
                }
            } else {
                for (int i3 = 0; i3 < stickerSets.size(); i3++) {
                    ...
                    if (!tL_messages_stickerSet.documents.isEmpty()) {
                        this.emojiList.document_id.add(Long.valueOf(((TLRPC.Document) tL_messages_stickerSet.documents.get(Math.abs(Utilities.fastRandom.nextInt() % tL_messages_stickerSet.documents.size()))).id));
                    }
                }
            }
        }
