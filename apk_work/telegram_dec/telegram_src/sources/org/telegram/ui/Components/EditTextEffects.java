...
public abstract class EditTextEffects extends EditText {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int i = 0; i < this.quoteBlocks.size(); i++) {
            this.quoteBlocks.get(i).draw(canvas, BitmapDescriptorFactory.HUE_RED, getWidth(), this.quoteColor, 1.0f, getPaint());
        }
        ...
        if (this.drawAnimatedEmojiDrawables && this.animatedEmojiDrawables != null) {
            ...
            AnimatedEmojiSpan.drawAnimatedEmojis(canvas, getLayout(), this.animatedEmojiDrawables, BitmapDescriptorFactory.HUE_RED, this.spoilers, computeVerticalScrollOffset() - AndroidUtilities.dp(6.0f), computeVerticalScrollOffset() + computeVerticalScrollExtent(), BitmapDescriptorFactory.HUE_RED, 1.0f, this.animatedEmojiColorFilter);
            canvas.restore();
        }
        canvas.restore();
        canvas.save();
