...
public class PaintToolsView extends LinearLayout {
    ...
    private RLottieImageView createView(boolean z, boolean z2) {
        ...
        int iDp = AndroidUtilities.dp(z ? BitmapDescriptorFactory.HUE_RED : 8.0f);
        int iDp2 = AndroidUtilities.dp(8.0f);
        if (!z2) {
            f = 8.0f;
        }
        rLottieImageView.setPadding(iDp, iDp2, AndroidUtilities.dp(f), AndroidUtilities.dp(8.0f));
    ...
    private float getOffsetForIndex(int i) {
        return i == this.brushesCount + 1 ? AndroidUtilities.dp(4.0f) : BitmapDescriptorFactory.HUE_RED;
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        float offsetForIndex = BitmapDescriptorFactory.HUE_RED;
        float f = rLottieImageView2 != null ? this.nextSelectedIndexProgress : BitmapDescriptorFactory.HUE_RED;
        float f2 = 1.0f;
        if (f > 0.25f && f < 0.75f) {
        ...
        if (i2 != -1) {
            offsetForIndex = getOffsetForIndex(i2);
        }
        canvas.drawCircle(AndroidUtilities.lerp(x, x2 + offsetForIndex, f), rLottieImageView.getY() + (rLottieImageView.getHeight() / 2.0f), fMin, this.selectorPaint);
    }
