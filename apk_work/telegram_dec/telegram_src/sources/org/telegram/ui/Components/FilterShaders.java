...
public class FilterShaders {
    ...
    private static class ToneCurve {
        ...
        private float[] getPreparedSplineCurve(ArrayList arrayList) {
            ...
            if (f > BitmapDescriptorFactory.HUE_RED) {
                for (int i2 = (int) f; i2 >= 0; i2--) {
                    arrayListSplineCurve.add(0, new PointF(i2, BitmapDescriptorFactory.HUE_RED));
                }
            }
            ...
            while (i < length - 1) {
                ...
                while (true) {
                    ...
                    if (i3 < ((int) f)) {
                        ...
                        Double.isNaN(d2);
                        Double.isNaN(d2);
                        float f4 = (float) ((d5 * d4) + (d6 * d3) + (((d2 * d2) / 6.0d) * (((((d4 * d4) * d4) - d4) * dArrSecondDerivative[i]) + ((((d3 * d3) * d3) - d3) * dArrSecondDerivative[i2]))));
                        if (f4 > 255.0f) {
                            f4 = 255.0f;
        ...
        private void updateToneCurveTexture() {
            ...
            for (int i = 0; i < 256; i++) {
                float f = i;
                int iMin = (int) Math.min(Math.max(this.redCurve[i] + f, BitmapDescriptorFactory.HUE_RED), 255.0f);
                int iMin2 = (int) Math.min(Math.max(this.greenCurve[i] + f, BitmapDescriptorFactory.HUE_RED), 255.0f);
                int iMin3 = (int) Math.min(Math.max(f + this.blueCurve[i], BitmapDescriptorFactory.HUE_RED), 255.0f);
                byteBufferAllocateDirect.put((byte) Math.min(Math.max(iMin3 + this.rgbCompositeCurve[iMin3], BitmapDescriptorFactory.HUE_RED), 255.0f));
                byteBufferAllocateDirect.put((byte) Math.min(Math.max(iMin2 + this.rgbCompositeCurve[iMin2], BitmapDescriptorFactory.HUE_RED), 255.0f));
                byteBufferAllocateDirect.put((byte) Math.min(Math.max(iMin + this.rgbCompositeCurve[iMin], BitmapDescriptorFactory.HUE_RED), 255.0f));
    ...
    public FilterShaders(boolean z, StoryEntry.HDRInfo hDRInfo) {
        ...
        floatBufferAsFloatBuffer3.put(fArr);
    ...
    public static FilterShadersDelegate getFilterShadersDelegate(final MediaController.SavedFilterState savedFilterState) {
        return new FilterShadersDelegate() { // from class: org.telegram.ui.Components.FilterShaders.1
            ...
            public float getTintHighlightsIntensityValue() {
                if (savedFilterState.tintHighlightsColor == 0) {
            ...
            public float getTintShadowsIntensityValue() {
                if (savedFilterState.tintShadowsColor == 0) {
                    return BitmapDescriptorFactory.HUE_RED;
                }
                return 0.5f;
            }
    ...
    */
    private boolean setupExternalShaders() {
        ...
        if (hDRType == 1) {
            ...
            String str = !z2 ? "#extension GL_OES_EGL_image_external : require" : "";
            String str2 = !z2 ? "samplerExternalOES" : "sampler2D";
            int[] iArr = new int[1];
            i2 = 0;
            while (true) {
                z = this.isVideo;
    ...
    public void drawCustomParamsPass() {
        ...
        if (filterShadersDelegate == null || filterShadersDelegate.shouldShowOriginal()) {
            ...
            GLES20.glUniform1f(this.skipToneHandle, zShouldDrawCurvesPass ? BitmapDescriptorFactory.HUE_RED : 1.0f);
    ...
    public void drawEnhancePass() {
        ...
        if (filterShadersDelegate == null || filterShadersDelegate.shouldShowOriginal()) {
            ...
            enhanceValue = BitmapDescriptorFactory.HUE_RED;
        } else {
            i = this.enhanceIntensityHandle;
            enhanceValue = this.delegate.getEnhanceValue();
        }
        GLES20.glUniform1f(i, enhanceValue);
    ...
    public void drawSharpenPass() {
        ...
        if (filterShadersDelegate == null || filterShadersDelegate.shouldShowOriginal()) {
            ...
            sharpenValue = BitmapDescriptorFactory.HUE_RED;
        } else {
            i = this.sharpenHandle;
            sharpenValue = this.delegate.getSharpenValue();
        }
        GLES20.glUniform1f(i, sharpenValue);
