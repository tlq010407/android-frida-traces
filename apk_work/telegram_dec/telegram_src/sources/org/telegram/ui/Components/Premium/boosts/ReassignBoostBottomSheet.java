...
public class ReassignBoostBottomSheet extends BottomSheetWithRecyclerListView {
    ...
    private static class AvatarHolderView extends FrameLayout {
        ...
        public AvatarHolderView(Context context) {
            ...
            addView(boostIconView, LayoutHelper.createFrame(28, 28.0f, 85, BitmapDescriptorFactory.HUE_RED, BitmapDescriptorFactory.HUE_RED, BitmapDescriptorFactory.HUE_RED, 3.0f));
    ...
    static class TopCell extends LinearLayout {
        ...
        public TopCell(Context context) {
            ...
            frameLayout.addView(frameLayout2, LayoutHelper.createFrame(-1, 70.0f, 0, BitmapDescriptorFactory.HUE_RED, BitmapDescriptorFactory.HUE_RED, BitmapDescriptorFactory.HUE_RED, BitmapDescriptorFactory.HUE_RED));
            ...
            addView(frameLayout, LayoutHelper.createLinear(-1, 70, BitmapDescriptorFactory.HUE_RED, 15.0f, BitmapDescriptorFactory.HUE_RED, BitmapDescriptorFactory.HUE_RED));
        ...
        public void showChats(List list, TLRPC.Chat chat) {
            ...
            if (this.addedChats.isEmpty() || this.addedChats.size() == 1) {
                i = 200;
                viewPropertyAnimatorTranslationX = this.avatarsContainer.animate().setInterpolator(cubicBezierInterpolator).translationX(BitmapDescriptorFactory.HUE_RED);
            } else {
                viewPropertyAnimatorTranslationX = this.avatarsContainer.animate().setInterpolator(cubicBezierInterpolator).translationX(AndroidUtilities.dp(11.5f) * (this.addedChats.size() - 1));
                i = 200;
            }
            viewPropertyAnimatorTranslationX.setDuration(i).start();
            this.toAvatar.animate().cancel();
            this.avatarsWrapper.animate().cancel();
            if (this.addedChats.isEmpty()) {
                ViewPropertyAnimator interpolator3 = this.avatarsWrapper.animate().setInterpolator(cubicBezierInterpolator);
                fDp = BitmapDescriptorFactory.HUE_RED;
                j = 200;
                interpolator3.translationX(BitmapDescriptorFactory.HUE_RED).setDuration(j).start();
            ...
            interpolator.translationX(fDp).setDuration(j).start();
        }
    }

    public ReassignBoostBottomSheet(BaseFragment baseFragment, TL_stories$TL_premium_myBoosts tL_stories$TL_premium_myBoosts, final TLRPC.Chat chat) {
        ...
        while (it.hasNext()) {
            ...
            TLRPC.Peer peer = tL_stories$TL_myBoost.peer;
    ...
    /* JADX INFO: Access modifiers changed from: private */
    public /* synthetic */ void lambda$new$1(TLRPC.Chat chat, final List list, final HashSet hashSet, final TL_stories$TL_premium_myBoosts tL_stories$TL_premium_myBoosts) {
        MessagesController.getInstance(this.currentAccount).getBoostsController().getBoostsStats(-chat.id, new Consumer() { // from class: org.telegram.ui.Components.Premium.boosts.ReassignBoostBottomSheet$$ExternalSyntheticLambda4
            ...
            public final void accept(Object obj) {
                this.f$0.lambda$new$0(tL_stories$TL_premium_myBoosts, list, hashSet, (TL_stories$TL_premium_boostsStatus) obj);
            }
        });
    }
    ...
    public static ReassignBoostBottomSheet show(BaseFragment baseFragment, TL_stories$TL_premium_myBoosts tL_stories$TL_premium_myBoosts, TLRPC.Chat chat) {
        ReassignBoostBottomSheet reassignBoostBottomSheet = new ReassignBoostBottomSheet(baseFragment, tL_stories$TL_premium_myBoosts, chat);
