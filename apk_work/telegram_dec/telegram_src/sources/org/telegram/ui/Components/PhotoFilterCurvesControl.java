...
public class PhotoFilterCurvesControl extends View {
    ...
    private void handlePan(int i, MotionEvent motionEvent) {
        ...
        if (i3 == 1) {
            curvesValue.blacksLevel = Math.max(BitmapDescriptorFactory.HUE_RED, Math.min(100.0f, curvesValue.blacksLevel + fMin));
        } else if (i3 == 2) {
            curvesValue.shadowsLevel = Math.max(BitmapDescriptorFactory.HUE_RED, Math.min(100.0f, curvesValue.shadowsLevel + fMin));
        } else if (i3 == 3) {
            curvesValue.midtonesLevel = Math.max(BitmapDescriptorFactory.HUE_RED, Math.min(100.0f, curvesValue.midtonesLevel + fMin));
        } else if (i3 == 4) {
            curvesValue.highlightsLevel = Math.max(BitmapDescriptorFactory.HUE_RED, Math.min(100.0f, curvesValue.highlightsLevel + fMin));
        } else if (i3 == 5) {
            curvesValue.whitesLevel = Math.max(BitmapDescriptorFactory.HUE_RED, Math.min(100.0f, curvesValue.whitesLevel + fMin));
        }
        invalidate();
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int i4 = 0; i4 < fArrInterpolateCurve.length / 2; i4++) {
            ...
            int i5 = i4 * 2;
            if (i4 == 0) {
                path.moveTo(f6 + (fArrInterpolateCurve[i5] * rect4.width), rect4.y + ((1.0f - fArrInterpolateCurve[i5 + 1]) * rect4.height));
            } else {
                path.lineTo(f6 + (fArrInterpolateCurve[i5] * rect4.width), rect4.y + ((1.0f - fArrInterpolateCurve[i5 + 1]) * rect4.height));
            }
        }
