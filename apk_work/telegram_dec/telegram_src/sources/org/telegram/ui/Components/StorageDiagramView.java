...
public abstract class StorageDiagramView extends View implements NotificationCenter.NotificationCenterDelegate {
    ...
    */
            ...
            if (clearViewData != null) {
                ...
                if (f8 != BitmapDescriptorFactory.HUE_RED) {
                    if (clearViewData.firstDraw) {
                        ...
                        double d4 = f10;
                        Double.isNaN(d4);
                        if (Math.abs((float) (d4 * ((3.141592653589793d * dWidth) / 180.0d))) <= 1.0f) {
                            float fCenterX = this.rectF.centerX();
                            double d5 = (-90.0f) - (360.0f * f7);
                            double dCos = Math.cos(Math.toRadians(d5));
                            Double.isNaN(dWidth);
                            float f11 = fCenterX + ((float) (dCos * dWidth));
                            float fCenterY = this.rectF.centerY();
                            double dSin = Math.sin(Math.toRadians(d5));
                            Double.isNaN(dWidth);
                            float f12 = fCenterY + ((float) (dWidth * dSin));
            ...
            if (clearViewData2 != null) {
                ...
                if (f14 != BitmapDescriptorFactory.HUE_RED) {
                    if (clearViewData2.firstDraw) {
                        ...
                        float f16 = f15 > BitmapDescriptorFactory.HUE_RED ? BitmapDescriptorFactory.HUE_RED : f15;
                        ...
                        double d6 = (dWidth2 * 3.141592653589793d) / d;
                        double d7 = f16;
                        Double.isNaN(d7);
                        if (Math.abs((float) (d7 * d6)) <= 1.0f) {
                            float fCenterX2 = this.rectF.centerX();
                            double d8 = (-90.0f) - (f13 * 360.0f);
                            double dCos2 = Math.cos(Math.toRadians(d8));
                            Double.isNaN(dWidth2);
                            float f17 = fCenterX2 + ((float) (dCos2 * dWidth2));
                            float fCenterY2 = this.rectF.centerY();
                            double dSin2 = Math.sin(Math.toRadians(d8));
                            Double.isNaN(dWidth2);
                            float f18 = fCenterY2 + ((float) (dWidth2 * dSin2));
                            if (Build.VERSION.SDK_INT >= 21) {
    ...
    protected void onMeasure(int i, int i2) {
        ...
        animatedTextDrawable.setGravity(i3);
        if (this.dialogText != null) {
            ...
            this.dialogTextLayout = StaticLayoutEx.createStaticLayout2(this.dialogText, this.dialogTextPaint, size2, Layout.Alignment.ALIGN_CENTER, 1.0f, BitmapDescriptorFactory.HUE_RED, false, TextUtils.TruncateAt.END, size2, 1);
        }
        ImageReceiver imageReceiver = this.avatarImageReceiver;
        if (imageReceiver != null) {
    ...
    public void setData(CacheModel cacheModel, ClearViewData[] clearViewDataArr) {
        ...
        update(false);
        this.singleProgress = this.enabledCount > 1 ? BitmapDescriptorFactory.HUE_RED : 1.0f;
    }
