...
public class ProfileBirthdayEffect extends View {
    ...
    public static class BirthdayEffectFetcher {
        ...
        public static BirthdayEffectFetcher of(int i, TLRPC.UserFull userFull, BirthdayEffectFetcher birthdayEffectFetcher) {
            ...
            int years = (userFull == null || (tL_account$TL_birthday = userFull.birthday) == null || (tL_account$TL_birthday.flags & 1) == 0) ? 0 : Period.between(LocalDate.of(tL_account$TL_birthday.year, tL_account$TL_birthday.month, tL_account$TL_birthday.day), LocalDate.now()).getYears();
            if (birthdayEffectFetcher != null) {
    ...
    static class ImageReceiverAsset extends ImageReceiver {
        ...
        /* renamed from: org.telegram.ui.ProfileBirthdayEffect$ImageReceiverAsset$1, reason: invalid class name */
    ...
    public void hide() {
        animate().alpha(BitmapDescriptorFactory.HUE_RED).setDuration(200L).setInterpolator(CubicBezierInterpolator.EASE_OUT_QUINT).start();
    }
    ...
    protected void onDraw(Canvas canvas) {
        if (this.fetcher.loaded) {
            ...
            if (this.isPlaying) {
                ...
                this.t = Utilities.clamp(this.t + (Utilities.clamp(jCurrentTimeMillis - this.lastTime, 20L, 0L) / 4200.0f), 1.0f, BitmapDescriptorFactory.HUE_RED);
                this.lastTime = jCurrentTimeMillis;
                updateSourcePoint();
                float filterWidth = EmojiAnimationsOverlay.getFilterWidth();
                this.fetcher.interactionAsset.setImageCoords((getWidth() - AndroidUtilities.dp(filterWidth)) / 2.0f, Math.max(BitmapDescriptorFactory.HUE_RED, this.sourcePoint.y - (AndroidUtilities.dp(filterWidth) * 0.5f)), AndroidUtilities.dp(filterWidth), AndroidUtilities.dp(filterWidth));
                canvas.save();
                ...
                while (size >= 0) {
                    ...
                    float interpolation = CubicBezierInterpolator.EASE_OUT_QUINT.getInterpolation(Utilities.clamp(fCascade / 0.4f, 1.0f, BitmapDescriptorFactory.HUE_RED));
