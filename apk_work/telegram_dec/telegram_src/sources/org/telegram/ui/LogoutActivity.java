...
public class LogoutActivity extends BaseFragment {
    ...
    public /* synthetic */ void lambda$createView$0(View view, int i, float f, float f2) {
            ...
            if (i == this.passcodeRow) {
                ...
                if (i != this.phoneRow) {
                    if (i == this.supportRow) {
                        dialogMakeLogOutDialog = AlertsCreator.createSupportAlert(this, null);
                    } else if (i != this.logoutRow || getParentActivity() == null) {
                        return;
                    } else {
                        dialogMakeLogOutDialog = makeLogOutDialog(getParentActivity(), this.currentAccount);
                    }
                    showDialog(dialogMakeLogOutDialog);
                }
        ...
        if (i2 > 0 && numValueOf != null) {
            actionIntroActivity = new LoginActivity(numValueOf.intValue());
            presentFragment(actionIntroActivity);
        } else {
            if (UserConfig.hasPremiumOnAccounts()) {
                return;
            }
            dialogMakeLogOutDialog = new LimitReachedBottomSheet(this, getContext(), 7, this.currentAccount, null);
            showDialog(dialogMakeLogOutDialog);
        }
    }
