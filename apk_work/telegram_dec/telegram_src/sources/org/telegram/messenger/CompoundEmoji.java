        ...
        public void draw(Canvas canvas) {
            ...
            if (this.newLeft != null) {
                if (this.leftUpdateT == null) {
                    this.leftUpdateT = new AnimatedFloat(BitmapDescriptorFactory.HUE_RED, new Runnable() { // from class: org.telegram.messenger.CompoundEmoji$CompoundEmojiDrawable$$ExternalSyntheticLambda0
            ...
            if (this.rightUpdateT == null) {
                this.rightUpdateT = new AnimatedFloat(BitmapDescriptorFactory.HUE_RED, new Runnable() { // from class: org.telegram.messenger.CompoundEmoji$CompoundEmojiDrawable$$ExternalSyntheticLambda0
                    @Override // java.lang.Runnable
                    public final void run() {
                        this.f$0.invalidate();
                    }
        ...
        if (str.startsWith("🤝")) {
            if (str.length() != 2) {
                if (str.length() == 4) {
                    ...
                    skinTone = skinTone >= 0 ? skinTone : -1;
                }
            }
            return new Pair<>(Integer.valueOf(skinTone), Integer.valueOf(skinTone));
        }
        skinTone = -1;
        String[] strArrSplit = str.split("\u200d");
        if (strArrSplit.length != 2 || !strArrSplit[0].startsWith("\u1faf1") || !strArrSplit[1].startsWith("\u1faf2")) {
            return null;
        }
        if (strArrSplit[0].length() != 2 && (strArrSplit[0].length() != 4 || (skinTone = getSkinTone(strArrSplit[0])) < 0)) {
            return null;
        }
        if (strArrSplit[1].length() == 2 || (strArrSplit[1].length() == 4 && (skinTone = getSkinTone(strArrSplit[1])) >= 0)) {
            return new Pair<>(Integer.valueOf(skinTone), Integer.valueOf(skinTone));
        }
        return null;
    }
