...
import org.telegram.tgnet.tl.TL_account$uploadRingtone;
    ...
    public void didReceivedNotification(int i, int i2, Object... objArr) {
        if (i == NotificationCenter.fileUploaded) {
            ...
            if (!this.canceled && str.equals(this.filePath)) {
                ...
                TL_account$uploadRingtone tL_account$uploadRingtone = new TL_account$uploadRingtone();
                tL_account$uploadRingtone.file = inputFile;
                tL_account$uploadRingtone.file_name = inputFile.name;
                String fileExtension = FileLoader.getFileExtension(new File(inputFile.name));
                tL_account$uploadRingtone.mime_type = fileExtension;
                tL_account$uploadRingtone.mime_type = "ogg".equals(fileExtension) ? "audio/ogg" : "audio/mpeg";
                ConnectionsManager.getInstance(this.currentAccount).sendRequest(tL_account$uploadRingtone, new RequestDelegate() { // from class: org.telegram.messenger.ringtone.RingtoneUploader$$ExternalSyntheticLambda0
